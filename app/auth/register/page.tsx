...
import { registerWithEmail, signInWithGoogle, mapAuthError } from '@/lib/auth';
...
export default function RegisterPage() {
  ...
  const FieldError = ({ msg }: { msg?: string }) =>
    msg ? <div className="flex items-center gap-1.5 mt-2 text-[#FF6B6B] font-body text-xs"><AlertCircle size={14} />{msg}</div> : null;
      ...
      >
        ...
        <div className="liquid-glass rounded-3xl p-8 md:p-10">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            {[
              { id: 'displayName', label: 'Display Name', type: 'text', placeholder: 'Stargazer', error: errors.displayName?.message },
              { id: 'email', label: 'Email', type: 'email', placeholder: 'you@example.com', error: errors.email?.message },
            ].map(f => (
              <div key={f.id} className="mb-5">
                <label className="block text-xs font-body font-medium text-muted uppercase tracking-widest mb-2" htmlFor={f.id}>{f.label}</label>
                <input 
                  id={f.id} 
                  type={f.type} 
            ...
            <div className="mb-5">
              ...
              <div className="relative">
                <input 
                  ...
                  {...register('password')} 
                />
                <button type="button" onClick={() => setShowPass(v => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-muted hover:text-text-primary transition-colors focus:outline-none">
            ...
            <div className="mb-8">
              <label className="block text-xs font-body font-medium text-muted uppercase tracking-widest mb-2" htmlFor="confirmPassword">Confirm Password</label>
              <input 
                id="confirmPassword" 
                type="password" 
                className="w-full bg-white/5 border border-stroke/50 rounded-xl p-3.5 text-text-primary font-body text-sm focus:outline-none focus:ring-1 focus:ring-accent focus:border-accent transition-all placeholder:text-text-primary/30" 
            ...
            <button type="submit" disabled={loading} className="w-full bg-accent-gradient text-bg rounded-xl p-3.5 flex items-center justify-center font-body font-medium text-sm transition-transform hover:scale-[1.02] active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:scale-100">
          ...
          <p className="text-center mt-8 text-sm text-muted font-body">
            ...
            <Link href="/auth/login" className="text-accent hover:text-white transition-colors">Sign in</Link>
          </p>
        </div>
      </motion.div>
    </div>
